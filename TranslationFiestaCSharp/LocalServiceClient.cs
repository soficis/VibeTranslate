...
namespace TranslationFiestaCSharp
{
    public class LocalServiceClient
    {
        ...
        {
            await EnsureAvailableAsync(cancellationToken).ConfigureAwait(false);
            using var response = await _http.PostAsync($"{_baseUrl}/models/verify", new StringContent("{}", System.Text.Encoding.UTF8, "application/json"), cancellationToken).ConfigureAwait(false);
            var body = await response.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode)
            {
                ThrowLocalError(body, (int)response.StatusCode);
        ...
        {
            await EnsureAvailableAsync(cancellationToken).ConfigureAwait(false);
            using var response = await _http.PostAsync($"{_baseUrl}/models/remove", new StringContent("{}", System.Text.Encoding.UTF8, "application/json"), cancellationToken).ConfigureAwait(false);
            var body = await response.Content.ReadAsStringAsync(cancellationToken).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode)
            {
                ThrowLocalError(body, (int)response.StatusCode);
