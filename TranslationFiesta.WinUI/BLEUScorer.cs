...
namespace TranslationFiesta.WinUI
{
    ...
    public class BLEUScorer
    {
        ...
        public TranslationQualityAssessment AssessTranslationQuality(string originalText, string backtranslatedText)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(originalText) || string.IsNullOrWhiteSpace(backtranslatedText))
                {
                    Logger.Warning("Empty text provided to AssessTranslationQuality, returning default assessment");
                    return new TranslationQualityAssessment
                    {
                        BleuScore = 0.0,
                        BleuPercentage = "0.00%",
                        ConfidenceLevel = "Low",
                        Description = "Unable to assess quality due to missing text",
                        QualityRating = "Poor",
                        Recommendations = "Please provide both original and translated text for quality assessment"
                    };
                }
            ...
            {
                Logger.Error($"Failed to assess translation quality: {ex.Message}", ex);
                return new TranslationQualityAssessment
                {
                    BleuScore = 0.0,
                    BleuPercentage = "N/A",
                    ConfidenceLevel = "Unknown",
                    Description = $"Quality assessment failed: {ex.Message}",
