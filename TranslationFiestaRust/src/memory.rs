...
impl TranslationMemory {
    ...
    pub fn lookup(
        ...
        let tx = conn.transaction().context("failed to begin lookup transaction")?;
    ...
    pub fn store(
        ...
        let mut conn = self.conn.lock().expect("translation memory lock poisoned");
        let tx = conn.transaction().context("failed to begin store transaction")?;
    ...
    pub fn clear(&self) -> Result<()> {
        let mut conn = self.conn.lock().expect("translation memory lock poisoned");
        let tx = conn.transaction().context("failed to begin clear transaction")?;
    ...
    fn init_schema(&self) -> Result<()> {
        let mut conn = self.conn.lock().expect("translation memory lock poisoned");
        let tx = conn.transaction().context("failed to begin init_schema transaction")?;
        ...
        tx.commit().context("failed to commit init_schema transaction")?;
        Ok(())
    }
}
